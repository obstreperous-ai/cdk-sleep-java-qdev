...
**Issue #5 Completed**: DynamoDB Metadata Table + State Machine I/O Handling implemented.
...
- ✅ DynamoDB table for storing audio pipeline metadata
- ✅ State machine input/output handling with DynamoDB integration

**Next Steps**: Issue #6 will add SNS notifications and basic error handling/status updates.
...
### Currently Implemented (Issues #3, #4, and #5)
...
#### 4. **DynamoDB Table** (`SleepAudioMetadataTable`)
- **Partition Key**: `audioId` (String)
- **Billing Mode**: PAY_PER_REQUEST (on-demand scaling)
- **Encryption**: AWS_MANAGED (server-side encryption)
- **Point-in-Time Recovery**: Enabled for data protection
- **Removal Policy**: DESTROY (for dev environment)
- **Purpose**: Stores metadata for each audio file processed through the pipeline
- **Attributes**:
  - `audioId`: Unique identifier (uses S3 object key)
  - `status`: Processing status (PROCESSING, COMPLETED, FAILED)
  - `inputBucket`: Source S3 bucket name
  - `inputKey`: Source S3 object key
  - `createdAt`: Timestamp when processing started
  - `updatedAt`: Timestamp of last update (future)

#### 5. **AWS Step Functions State Machine** (`SleepAudioPipelineStateMachine`)
...
- **IAM Role**: Least privilege with scoped permissions for DynamoDB, Polly, S3 read/write
  - **Polly Task** (`PollyTextToSpeech`): Invokes Amazon Polly `SynthesizeSpeech` API
  - **DynamoDB Write Task** (`WriteInitialMetadata`): Writes initial metadata record with PROCESSING status
...
- **Future Enhancements**: Will add S3 object retrieval, error handling, and status updates
...
## Current Architecture Diagram (Issue #5 Implementation)
...
graph LR
    ...
    EventBridge -->|"3. Start Execution<br/>(with S3 details)"| StepFunctions["🔀 Step Functions<br/><b>SleepAudioPipelineStateMachine</b><br/>━━━━━━━━━━━<br/>✅ Standard Workflow<br/>✅ CloudWatch Logging<br/>✅ X-Ray Tracing<br/>✅ DynamoDB + Polly"]
    Polly -->|"5. Audio Stream"| StepFunctions
    StepFunctions -->|"4. Write Metadata"| DynamoDB[("💾 DynamoDB<br/><b>SleepAudioMetadataTable</b><br/>━━━━━━━━━━━<br/>✅ On-Demand Billing<br/>✅ Encrypted<br/>✅ PITR Enabled<br/>• Status: PROCESSING")]
    
    StepFunctions -->|"5. Synthesize Speech"| Polly["🎙️ Amazon Polly<br/><b>Neural TTS</b><br/>━━━━━━━━━━━<br/>• Voice: Joanna<br/>• Format: MP3<br/>• Engine: Neural"]
    StepFunctions -->|"6. Processing Complete"| SuccessState["✅ Success State<br/>(ProcessingComplete)"]
    Polly -->|"6. Audio Stream"| StepFunctions
    ...
    StepFunctions -->|"7. Processing Complete"| SuccessState["✅ Success State<br/>(ProcessingComplete)"]
    ...
    style DynamoDB fill:#f39c12,stroke:#e67e22,stroke-width:3px,color:#fff
...
3. **Test Coverage**:
   ...
   - DynamoDB table with proper key schema, billing mode, encryption, and PITR
   - State machine with DynamoDB permissions
...
- DynamoDB table: `SleepAudioMetadataTable`
- State machine: `SleepAudioPipelineStateMachine`
  - DynamoDB write task: `WriteInitialMetadata`
...
2. Writes initial metadata record to DynamoDB with status=PROCESSING
3. Invokes Polly `SynthesizeSpeech` with placeholder text
4. Completes successfully (future: will write to S3 and update DynamoDB status)
```
...
- ✅ Step Functions execution role with scoped access to:
  - `dynamodb:PutItem`, `dynamodb:GetItem`, `dynamodb:UpdateItem` (for metadata storage)
...
- ✅ Step Functions execution role follows least privilege:
  - Only necessary DynamoDB actions (PutItem, GetItem, UpdateItem)
  - Only necessary Polly actions
  - DynamoDB table permissions scoped to specific table
...
**Current Infrastructure Cost (Issue #5):**
...
**Total Current: ~$2-3/month for basic infrastructure (light usage)**
**Total Current: ~$1-2/month for basic infrastructure (light usage)**
**Note**: Actual costs depend on usage. Polly and DynamoDB charges apply when state machine executes.
...
- Lambda, Bedrock will add costs based on usage
- SNS notifications: minimal ($0.50 per million notifications)
...
**Last Updated**: Issue #5 - TDD Implementation of DynamoDB Metadata Table + State Machine I/O Handling  
**Next Review**: After Issue #6 (TDD: SNS Notifications + Basic Error Handling & Status Updates)  
